...
function renderReveal() {
  ...
  selected.forEach((card, index) => {
    ...
    const art     = imageByCard[card.id] || imageByCard.mirror;
    const fallback = `https://picsum.photos/seed/${encodeURIComponent(card.id)}/900/1350`;
    ...
    // Надёжный fallback: слушатель вместо onerror-атрибута
    cardEl.querySelector("img").addEventListener("error", (e) => {
    ...
    cardEl.onmouseleave = () => {
      cardEl.querySelector(".card-inner").style.transform = "rotateY(0deg)";
    };
...
// ===== Подсказки (нажатие сразу переходит к колоде) =====
...
saveBtn.onclick = () => {
  ...
  const summary = selected.map((c) => `${c.roleText}: ${c.name}. ${c.advice}`).join("\n");
  ...
  navigator.clipboard.writeText(
    ...
    () => setStatus("✓ Расклад скопирован в буфер"),
    () => setStatus("Скопируйте расклад вручную")
  );
};
